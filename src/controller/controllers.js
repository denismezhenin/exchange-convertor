...
export const currencyFrom = document.querySelector(
  ".form__fromCurrency__select"
);
...
export const getRatesForSelectedCurrencies = async () => {
  try {
    ...
    const data = await fetch(
      `${API}${endpoint}?access_key=${access_key}&symbols=${currencyFrom.value},${currencyTo.value}&format=1`
    );
...
const setToCurrency = () => {
  const result = (
    ...
    (state.fromCurrencyRate / state.toCurrencyRate)
  ).toFixed(2);
